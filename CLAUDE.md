...
Claude Code теряет контекст между сессиями. claude-memory-kit даёт ему долговременную память: решения (ADR), задачи (roadmap), исследования (docs), граф зависимостей (/context). 20 файлов, один промпт для установки.
...
- **Sessions deep dive**: sessions.md — архив с глубиной. При недопонимании, споре о контексте решения или вопросе "почему мы это отвергли?" — читай более ранние блоки. Ищи по ключевым словам в строке `Темы:`. Не ограничивайся последним блоком.
